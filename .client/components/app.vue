<template>
  <section class="todoapp">
    ...
    <footer class="footer" v-show="state.todos.length" v-cloak>
      ...
      <button class="clear-completed" v-on="click:removeCompleted" v-show="state.todos.length > remaining">
        Clear completed
      </button>
    </footer>
  </section>
...
export default {
  mixins: [vuedux.mixin.bindActions({...TodoActions, ...CollectionActions})],
  data() {
    return {
      newTodo: '',
      editedTodo: null,
  ...
  computed: {
    filteredTodos(){ return filters[this.visibility](this.state.todos);},
    remaining(){ return filters.active(this.state.todos).length;},
    allDone: {
